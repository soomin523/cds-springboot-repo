...
//@RestController
//@RequiredArgsConstructor //lombok 의존 자동 주입(생성자 방식)
//public class TodoController2 {
//	
//	//메소드 내에서 사용되는 클래스에 대한 개체 주입을 생성자 방식을 이용해서 자동으로 주입받을 수 있도록 함
//	//lombok API에서 @RequiredArgsConstructor 어노테이션을 클래스 위에 붙이고 필드에는 final을 붙여줌
//	private final TodoService todoService;
//	
//	//이하 구문을 lombok API에서 @RequiredArgsConstructor 어노테이션을 사용함으로써 대체함
...
//	public String insert(@RequestBody TodoDTO dto) {
//		//@RequestBody: 사용자로부터 전달되는 데이터가 HTTP의 body 부분에 포함되어졌을 경우 그것을 받는 객체임을 나타내줌
//		String result = "FAIL"; //입력 실패 시 결과값
//		
//		//사용자로부터 받은 데이터를 DB에 저장하려면 TodoServiceImpl 클래스를 이용해야 함
//		//스프링으로부터 의존주입(DI)을 받아서 이용함
...
//	//URL을 통해서 들어오는 값을 변수로 사용할 수 있도록 해주는 경로변수 : {경로변수명}으로 정의함
//	//요청을 처리하는 메소드의 매개변수로 저장할 때는 @PathVariable("경로변수명")으로 사용함
...
//		String result = "FAIL";//할일 변경 실패 시 결과값
//		
//		try {
...
//	public String deleteTodo(@PathVariable("id2")String id2) {
//		String result = "FAIL";
//		
//		String id = "ff8080"+id2;
//		
